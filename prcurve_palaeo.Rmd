...
```{r libraries, echo=FALSE, cache=FALSE, results='hide'}
...
```{r legendre-data, cache=TRUE, echo=FALSE, results='hide'}
...
```{r legendre-data-plot, echo=FALSE, cache=TRUE, dependson='legendre-data', fig.height=6, fig.width=7, fig.cap="\\label{legendre-data-fig}"}
...
```{r abernethy-strat-fig, echo=FALSE, fig.height=5, fig.width=7, fig.cap="\\label{abernethy-strat}"}
data(abernethy)
Stratiplot(Age ~ . - Depth, data = chooseTaxa(abernethy, n.occ = 5, max.abun = 10),
           type = c("g","l","h"), lwd.h = 1, sort = "wa", labelAt = 0, labelRot = 60)
```

```{r abernethy-pca-setup, echo=FALSE, results='hide', cache=TRUE}
...
```{r abernethy-pca-plot, echo=FALSE, cache=TRUE, dependson='abernethy-pca-setup', fig.width=7, fig.height=4, fig.cap="\\label{abernethy-pca-fig}"}
...
```{r comparison-figure, echo=FALSE, cache=TRUE, fig.height=7, fig.width=7, fig.cap="\\label{PCcomparison}Relationship between (a) linear regression, (b) principal components analysis, (c) smoothing splines, and (d) principal curves each applied a simple 2-dimensional data set."}
set.seed(123)
N <- 50
x <- seq(0, 1, length = N)
...
```{r iterations-prcurve, echo=FALSE, results='hide', dependson="comparison-figure", cache=TRUE}
...
```{r iterations-figure, echo=FALSE, dependson='iterations-prcurve', fig.height=7, fig.width=7, fig.cap="\\label{iterations-fig}"}
layout(matrix(1:4, ncol = 2, byrow = TRUE))
...
```{r packing-model-data, echo=FALSE, cache = TRUE}
require("coenocliner")
...
gh <- rep(10, gspp)
set.seed(1)
gsimMu <- sim1dNegbinom(gloc, gopt, gtol, gh, alpha = 1, expectation = TRUE)
set.seed(1)
gsim <- sim1dNegbinom(gloc, gopt, gtol, gh, alpha = 1, expectation = FALSE)
```

```{r packing-model-figure, echo=FALSE, dependson='packing-model-data', fig.height=6, fig.width=8, fig.cap="\\label{packing-model-fig}"}
layout(matrix(1:2, ncol = 2))
matplot(gloc, gsimMu, type = "l", lty = "solid")
matplot(gloc, gsim, type = "l", lty = "solid")
layout(1)
```
# Methods
